...
export function projectsReducer(state = initState, action: Action): ProjectsState {
  switch (action.type) {
    case ProjectsActionTypes.GET_All_PROJECTS:
      return { ...state, dataState: ProjectsStateEnum.LOADING, currentAction: <ProjectsActions>action }
    case ProjectsActionTypes.GET_All_PROJECTS_SUCCESS:
      const projects = (<ProjectsActions>action).payload;
      const todo = projects.filter((project: Project) => project.projectStatus === ProjectStatus.ON_HOLD);
      const done = projects.filter((project: Project) => project.projectStatus === ProjectStatus.RUNNING);
      const finished = projects.filter((project: Project) => project.projectStatus === ProjectStatus.FINISHED);
    ...
    case ProjectsActionTypes.GET_All_PROJECTS_ERROR:
      return { ...state, dataState: ProjectsStateEnum.ERROR, errorMessage: (<ProjectsActions>action).payload, currentAction: <ProjectsActions>action }
    ...
    case ProjectsActionTypes.DELETE_PROJECT_SUCCESS:
      ...
      list.splice(index, 1);
      return { ...state, dataState: ProjectsStateEnum.DELETED, projects: list, currentProject: null, currentAction: <ProjectsActions>action }
    ...
    case ProjectsActionTypes.NEW_PROJECT_SUCCESS:
      return { ...state, dataState: ProjectsStateEnum.NEW, currentProject: null, currentAction: <ProjectsActions>action }
    ...
    case ProjectsActionTypes.SAVE_PROJECT_SUCCESS:
      ...
      return { ...state, dataState: ProjectsStateEnum.LOADED, projects: projs, currentAction: <ProjectsActions>action }
    ...
    case ProjectsActionTypes.EDIT_PROJECT_SUCCESS:
      return { ...state, dataState: ProjectsStateEnum.LOADED, currentProject: (<ProjectsActions>action).payload, currentAction: <ProjectsActions>action }
