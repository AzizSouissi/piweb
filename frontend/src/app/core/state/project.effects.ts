...
import { DeleteProjectActionError, DeleteProjectActionSuccess, EditProjectActionError, EditProjectActionSuccess, GetAllProjectsActionError, GetAllProjectsActionSuccess, NewProjectActionSuccess, ProjectsActionTypes, ProjectsActions, SaveProjectActionError, SaveProjectActionSuccess, UpdateProjectActionError, UpdateProjectActionSuccess } from './project.actions';

@Injectable()
export class ProjectsEffects {
  constructor(private projectService: ProjectManagementService, private effectActions: Actions) {
  }
  ...
  deleteProjectEffect: Observable<ProjectsActions> = createEffect(() =>
    this.effectActions.pipe(
      ...
      mergeMap((action : ProjectsActions) => {
        return this.projectService.deleteProject(action.payload.id)
          .pipe(
  ...
  editProjectEffect: Observable<ProjectsActions> = createEffect(() =>
    this.effectActions.pipe(
      ...
      mergeMap((action : ProjectsActions) => {
        return this.projectService.getProjectById(action.payload)
          .pipe(
