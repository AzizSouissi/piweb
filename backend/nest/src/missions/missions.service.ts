...
export class MissionService {
  constructor(
    ...
    private enterpriseService: EntreprisesService,
  ...
  async assignUserToMission(
    ...
    const isdisponible = await this.TaskService.isUserDisponible(
      user._id,
      datemission,
    );
    console.log('isdisponible', isdisponible);
    const isdisponiblemission = await this.isUserAvailableForMission(
      user._id,
      datemission,
    );
    ...
    if (isdisponible && isdisponiblemission) {
      ...
      console.log('Cet employé est indisponible dans cette date');
      throw new HttpException(
        'Cet employé est indisponible dans cette date',
        400,
      );
    }
  }
  ...
  async getUsersAvailable(date: string): Promise<User[]> {
    ...
    for (const user of users) {
      const isAvailable = await this.TaskService.isUserDisponible(
        user.id,
        date,
      ); // Utilisez votre fonction isUserDisponible pour vérifier la disponibilité de chaque utilisateur
      if (isAvailable) {
